...
public class JwtTokenService implements TokenService {
    ...
    public ResponseCookie logoutFromRequest(HttpServletRequest request) {
        try {
            if (request.getCookies() != null) {
                for (Cookie c : request.getCookies()) {
                    if (cookieName().equals(c.getName())) {
                        String refreshJwtFromCookie = c.getValue();
            ...
            log.warn("Failed to logout refresh token", e);
        }
    ...
    private String cookieName() {
        return Optional.ofNullable(jwtProperties.getCookie().getName()).orElse("refreshToken");
    }

    private String cookiePath() {
        return Optional.ofNullable(jwtProperties.getCookie().getPath()).orElse("/");
    }

    private String cookieSameSite() {
        return Optional.ofNullable(jwtProperties.getCookie().getSameSite()).orElse("Lax");
    }
    ...
    private int accessTtl() {
        return jwtProperties.getAccess().accessExpirationSeconds();
    }
    ...
    private String accessIssuer() {
        return Optional.ofNullable(jwtProperties.getAccess().getIssuer()).orElse("medflow");
    }

    private String refreshIssuer() {
        return Optional.ofNullable(jwtProperties.getRefresh().getIssuer()).orElse("medflow");
    }

    private String audience() {
        return Optional.ofNullable(jwtProperties.getAudience()).orElse("medflow-api");
    }

    private int clockSkewSeconds() {
        return jwtProperties.getClockSkewSeconds() > 0 ? jwtProperties.getClockSkewSeconds() : 120;
    }
